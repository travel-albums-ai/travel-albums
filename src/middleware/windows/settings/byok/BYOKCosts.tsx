          ...
          <Box key={index} sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 0.5, p: 0.25, justifyContent: 'space-between', flexWrap: 'nowrap', borderBottom: 1, borderColor: 'divider' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1, alignItems: 'center' }}>
              {/* <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, justifyContent: 'space-between' }}>
                <SolidChip count={usageStat.usage.input_tokens / 1000} label={`M Input`} icon={<LogIn size={16} />} borderless />
                <SolidChip count={usageStat.usage.output_tokens / 1000} label={`M Output`} icon={<LogOut size={16} />} borderless />
              </Box> */}
              <SolidChip count={usageStat.usage.total_tokens / 1000} label={`M tokens`} icon={<BoxIcon size={16} />} fontSize={13} />
            </Box>
          </Box>
        ))}
