...
export default function IndexerSettings({ asIs = false }: { asIs?: boolean }) {
  ...
  const DOM = <>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'space-between', mb: 1, flexDirection: 'column' }}>
      <SettingsGeneralRow icon={<Folder />} label="Takeout sources">
        <Box sx={{ width: '350px', display: 'flex', flexDirection: 'column', gap: 1, justifyContent: 'space-between' }}>
  ...
  return <>
    {asIs ? DOM : <SettingsSection title="Path to cache and photos archive" icon={<Code />}>
      {DOM}
    </SettingsSection>}
  </>
}
